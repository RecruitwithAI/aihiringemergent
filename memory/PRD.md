...
- ✅ **Feb 6, 2026** — **Security hardening trio (20/20 backend tests passed, iteration_7.json)**:
  1. **At-rest encryption of OpenAI API keys** — `utils/crypto.py` (Fernet AES-128-CBC + HMAC, `enc:v1:` sentinel prefix). `users.openai_api_key` now stored as ciphertext; decrypted only at LLM call time. `utils/key_migration.py` transparently encrypts any pre-existing plaintext rows on startup. New env var `API_KEY_ENCRYPTION_KEY` in backend/.env.
  2. **Security headers middleware** — `server.py :: SecurityHeadersMiddleware` adds 6 headers to every response: `Strict-Transport-Security` (2yr+preload), `X-Content-Type-Options: nosniff`, `X-Frame-Options: SAMEORIGIN` (preserves Emergent iframe), `Referrer-Policy: strict-origin-when-cross-origin`, `Permissions-Policy` (camera/mic/geo/payment denied), `Cross-Origin-Opener-Policy: same-origin`. CSP intentionally NOT set server-side (would conflict with React inline styles + data URLs).
  3. **Login brute-force lockout** — new `login_attempts` collection (compound `ix_email_ts` + 1h TTL `ttl_attempt`); `utils/login_throttle.py` enforces 5 failed attempts per email in a 15-min rolling window → 15-min lockout returning HTTP 429 (even correct password is rejected during lockout). Successful login clears the counter. Case-insensitive email normalization prevents bypass. Lockout check runs BEFORE bcrypt to prevent CPU-DoS amplification.
  4. (Polish) `/api/ai/generate` now maps upstream provider errors → HTTP 429 (rate-limit/quota) / 401 (bad key) / 502 (other) instead of raw 500.
- ✅ **Feb 6, 2026** — Profile Picture upload/edit/remove (data-URL stored in `users.picture`):
  - Backend: `PUT /api/users/me/picture` + `DELETE /api/users/me/picture` (`routers/users.py`), `UserPictureUpdate` schema with mime/size validation (≤400KB data URL, jpeg/png/webp)
  - Frontend: `components/AvatarUploadModal.js` (react-easy-crop, round crop, zoom slider, 256×256 JPEG output @ q=0.85), Profile Picture section in `pages/ProfileSettings.js`
  - **Zero-touch propagation**: `UserAvatar` already read `user.picture` and `AUTHOR_LOOKUP` already projected it → picture renders automatically across Navbar, Dashboard activity feed, Recent Challenges, Challenges list, Challenge Detail, Leaderboard, Admin User Table, Profile
  - Smoke-tested: PUT/DELETE return updated user; invalid data URL rejected with 422; cropper modal opens correctly
