          ...
          {tools.map((tool) => {
            ...
            return (
              <button
                ...
                className={`${CARD} p-6 text-left transition-all duration-300 hover:scale-[1.02] hover:border-${tool.color.replace('text-', '')}/30 hover:bg-white/[0.06] group`}
                data-testid={`tool-${tool.id}`}
              >
                <div className={`w-12 h-12 rounded-xl ${tool.bg} border ${tool.border} flex items-center justify-center mb-4 group-hover:scale-110 transition-transform duration-300`}>
                  <Icon className={`w-6 h-6 ${tool.color}`} strokeWidth={1.5} />
                </div>
                <h3 className="text-lg font-semibold text-white mb-2 font-[Lexend]">
                  {tool.label}
                </h3>
