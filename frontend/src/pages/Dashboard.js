...
import { Brain, MessageSquare, Trophy, Users, ArrowRight, Zap, Star, TrendingUp } from "lucide-react";
    ...
    <div className="min-h-screen bg-[#FAFAF9]" data-testid="dashboard-page">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4 auto-rows-[minmax(140px,auto)]">
          {/* XP Card — 2 cols */}
          <div className="md:col-span-2 bg-white rounded-xl border border-stone-200 shadow-sm p-6 card-hover animate-float-in" data-testid="xp-card">
            <div className="flex items-center justify-between mb-4">
              <div>
                <p className="text-sm font-medium text-stone-500 uppercase tracking-wide">Your Progress</p>
                <div className="flex items-center gap-3 mt-1">
                  <span className="text-3xl font-semibold text-stone-900 font-[Lexend]">{points}</span>
              ...
              <div className="text-right">
                ...
                <p className="text-2xl font-semibold text-primary font-[Lexend]" data-testid="user-rank">#{stats?.user_rank || "-"}</p>
              </div>
            </div>
            {next.name !== "Max" && (
              <div>
                ...
                <div className="w-full bg-stone-100 rounded-full h-2.5">
                  <div className="xp-bar h-2.5" style={{ width: `${Math.min(next.progress, 100)}%` }} data-testid="xp-progress-bar" />
                </div>
              </div>
            )}
          </div>
          ...
          {/* Stat cards */}
          <div className="bg-white rounded-xl border border-stone-200 shadow-sm p-6 card-hover animate-float-in stagger-1" data-testid="members-stat">
            <Users className="w-5 h-5 text-primary mb-3" strokeWidth={1.5} />
            <p className="text-2xl font-semibold text-stone-900 font-[Lexend]">{stats?.total_members || 0}</p>
            <p className="text-sm text-stone-500">Community Members</p>
          </div>
          <div className="bg-white rounded-xl border border-stone-200 shadow-sm p-6 card-hover animate-float-in stagger-2" data-testid="challenges-stat">
            <MessageSquare className="w-5 h-5 text-emerald-500 mb-3" strokeWidth={1.5} />
            <p className="text-2xl font-semibold text-stone-900 font-[Lexend]">{stats?.total_challenges || 0}</p>
            <p className="text-sm text-stone-500">Active Challenges</p>
          </div>

          {/* Quick Actions */}
          {quickActions.map((a, i) => (
            <Link key={a.label} to={a.href} className={`bg-white rounded-xl border border-stone-200 shadow-sm p-6 card-hover group animate-float-in stagger-${i + 1}`} data-testid={`quick-action-${a.label.toLowerCase().replace(" ", "-")}`}>
              <div className={`w-10 h-10 rounded-lg ${a.color} border flex items-center justify-center mb-3 group-hover:scale-110 transition-transform duration-200`}>
              ...
              <ArrowRight className="w-4 h-4 text-stone-300 group-hover:text-primary mt-2 transition-colors duration-200" strokeWidth={1.5} />
            </Link>
          ))}

          {/* Recent Challenges — 2 cols */}
            ...
            {stats?.recent_challenges?.length > 0 ? (
              <div className="space-y-3">
                {stats.recent_challenges.slice(0, 4).map((c) => (
                  <Link key={c.challenge_id} to={`/challenges/${c.challenge_id}`} className="flex items-center gap-3 p-3 rounded-lg hover:bg-stone-50 transition-colors duration-200 group">
                    ...
                    <TrendingUp className="w-4 h-4 text-stone-300 flex-shrink-0" strokeWidth={1.5} />
                  </Link>
                ))}
              </div>
            ) : (
          ...
          {/* Points Guide */}
          <div className="md:col-span-1 lg:col-span-2 bg-primary/5 rounded-xl border border-primary/10 p-6 animate-float-in stagger-4" data-testid="points-guide">
            <h3 className="font-semibold text-stone-900 font-[Lexend] mb-3">How to Earn XP</h3>
