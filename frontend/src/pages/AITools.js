...
const TOOLS = [
  ...
  { id: "search-strategy", icon: Search, title: "Search Strategy", desc: "Create comprehensive candidate search strategies", placeholder: "Who are you looking for? e.g. 'CMO for a D2C healthcare brand expanding to Europe...'", color: "bg-blue-50 text-blue-600 border-blue-100" },
  { id: "candidate-research", icon: UserCheck, title: "Candidate Research", desc: "Deep research on candidate profiles", placeholder: "Describe the candidate or profile, e.g. 'VP of Sales at a SaaS unicorn, 15 years experience...'", color: "bg-emerald-50 text-emerald-600 border-emerald-100" },
  { id: "dossier", icon: FolderOpen, title: "Candidate Dossier", desc: "Create presentation-ready candidate dossiers", placeholder: "Provide candidate details, e.g. 'John Smith, CTO at TechCorp, 20 years in enterprise software...'", color: "bg-amber-50 text-amber-600 border-amber-100" },
  { id: "client-research", icon: Building2, title: "Client Research", desc: "Research potential client companies", placeholder: "Describe the company, e.g. 'Acme Corp, mid-market cybersecurity firm, recently funded Series C...'", color: "bg-rose-50 text-rose-600 border-rose-100" },
];
...
export default function AITools() {
  ...
  const downloadAsText = () => {
    ...
    const a = document.createElement("a"); a.href = url; a.download = `${selectedTool?.id}-output.txt`; a.click();
      ...
      <div className="min-h-screen bg-[#FAFAF9]" data-testid="ai-tools-page">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="mb-8 animate-float-in">
            <h1 className="text-3xl md:text-4xl font-semibold tracking-tight text-stone-900 font-[Lexend]">AI Tools Hub</h1>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {TOOLS.map((tool, i) => (
              <button
                ...
                className={`text-left bg-white rounded-xl border border-stone-200 shadow-sm p-6 card-hover tool-card-gradient group animate-float-in stagger-${i + 1}`}
                data-testid={`tool-card-${tool.id}`}
              >
                <div className={`w-12 h-12 rounded-xl ${tool.color} border flex items-center justify-center mb-4 group-hover:scale-110 transition-transform duration-200`}>
                ...
                <p className="mt-4 text-xs text-primary font-medium opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                  Use tool &rarr;
                </p>
              </button>
            ))}
  ...
  return (
    <div className="min-h-screen bg-[#FAFAF9]" data-testid="ai-tool-workspace">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white rounded-xl border border-stone-200 shadow-sm p-6 md:p-8 animate-float-in">
          ...
          <div className="space-y-4">
            <div>
              ...
              <Textarea data-testid="ai-tool-prompt" placeholder={selectedTool.placeholder} value={prompt} onChange={(e) => setPrompt(e.target.value)} className="min-h-[120px] resize-none" />
            </div>
            <div>
              ...
              <Input data-testid="ai-tool-context" placeholder="Any extra details, company info, or preferences..." value={context} onChange={(e) => setContext(e.target.value)} />
            </div>
            <Button data-testid="ai-generate-btn" onClick={handleGenerate} disabled={generating || !prompt.trim()} className="rounded-full px-8 bg-primary hover:bg-primary/90 text-white">
        ...
        {(result || generating) && (
          <div className="mt-6 bg-white rounded-xl border border-stone-200 shadow-sm p-6 md:p-8 animate-float-in" data-testid="ai-result-panel">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-semibold text-stone-900 font-[Lexend]">Generated Output</h3>
              {result && (
            ...
            {generating ? (
              <div className="flex items-center justify-center py-12">
                ...
                <span className="text-stone-400">Generating with GPT-5.2...</span>
              </div>
            ) : (
